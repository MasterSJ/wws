...
public class Lunar {
    ...
    public void e2c(String strTime) throws ParseException {
    	Date date = chineseDateFormat.parse(strTime);
    	e2c(date);
    }
    
    // 将阳历向阴历转换
    @SuppressWarnings("deprecation")
    public void e2c(Date date) throws ParseException {
        TheDate = date;;
        int total, m, n, k;
        boolean isEnd = false;
    ...
    // 整理输出
    public String getcDateString(boolean showYear, boolean showSx, 
    		boolean showMonth, boolean showDate, boolean showHour) {
        String tmp = "";
    ...
    /*获取农历表示日期*/
    public String getLunar(String date, boolean showLeap) {
        try {
            e2c(date);
            String str = getcDateString(false, false, true, true, false);
            if (!showLeap && str.contains("闰")) {
    ...
    /*获取农历表示日期*/
    public String getLunar(Date date, boolean showLeap) {
        try {
            e2c(date);
            String str = getcDateString(false, false, true, true, false);
            if (!showLeap && str.contains("闰")) {
    ...
    /*获取date日期之后several天的日期*/
    public Date getSeveralDayLater(Date date, int several) {
        Calendar c = Calendar.getInstance();  
        c.setTime(date);  
        c.add(Calendar.DAY_OF_YEAR, several);
        return c.getTime();
    }
    
    public static void main(String[] args) {
        Lunar lunar = new Lunar();
        Date date = lunar.getSeveralDayLater(new Date(), -360);
        String str = lunar.getLunar(date, true);
        System.out.println(str);
        str = lunar.getLunar(date, false);
        System.out.println(str);
    }
}